...
class EmployeController extends AbstractController
{
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        if (array_key_exists('actif', $data)) { $employe->setActif((bool) $data['actif']); }

        if (array_key_exists('photo', $data)) {  $employe->setPhoto($data['photo']); }

        if (array_key_exists('biometriqueData', $data)) {
            if (!is_array($data['biometriqueData']) && $data['biometriqueData'] !== null) {
                throw new ApiException(
                    'Les données envoyées sont invalides.',  422, 'VALIDATION_ERROR', ['biometriqueData' => ['Ce champ doit être un tableau ou null.']]
                );
            }
    ...
    public function uploadPhoto(int $id, Request $request): JsonResponse
    {
        ...
        if (!$file) {
            throw new ApiException( 'Aucune photo envoyée.', 400, 'PHOTO_FILE_REQUIRED', ['file' => ['Le champ attendu est : file.']]
            );
        }
    ...
    public function update(int $id, Request $request): JsonResponse
    {
        ...
        if (array_key_exists('nom', $data)) { $employe->setNom($data['nom']); }

        if (array_key_exists('prenom', $data)) { $employe->setPrenom($data['prenom']); }

        if (array_key_exists('email', $data)) { $employe->setEmail($data['email']); }

        if (array_key_exists('matricule', $data)) { $employe->setMatricule($data['matricule']); }

        if (array_key_exists('poste', $data)) { $employe->setPoste($data['poste']); }

        if (array_key_exists('dateEmbauche', $data)) { $employe->setDateEmbauche($this->parseDate($data['dateEmbauche'], 'dateEmbauche', 'Y-m-d')); }

        if (array_key_exists('telephone', $data)) { $employe->setTelephone($data['telephone']); }
        ...
        if (array_key_exists('departementId', $data)) {
            if ($data['departementId'] === null || $data['departementId'] === '') {
                throw new ApiException(
                    'Le département est obligatoire.',  422, 'VALIDATION_ERROR', ['departementId' => ['Ce champ ne peut pas être vide.']]
                );
            }
    ...
    {
        $employe = $this->employeRepository->find($id);
        $this->assertFound($employe, 'Employe non trouvé.', 'EMPLOYE_NOT_FOUND');
        ...
        if (!is_array($data['descriptors'])) {
            throw new ApiException(
                'Les donnees envoyes sont invalides.',
                422,
                'VALIDATION_ERROR',
...
public function create(Request $request): JsonResponse
{
    ...
    if ($photo) {
        try {
            $filename = $this->photoUploadService->upload($photo);
            $employe->setPhoto($filename);
        } catch (\InvalidArgumentException $exception) {
            throw new ApiException($exception->getMessage(), 422, 'VALIDATION_ERROR');
        }
